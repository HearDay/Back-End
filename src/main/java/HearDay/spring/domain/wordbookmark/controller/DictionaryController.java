...
public class DictionaryController {
    ...
    public ResponseEntity<CommonApiResponse<List<String>>> searchWord(@PathVariable String word) {
        List<String> results = dictionaryService.searchWord(word);

        return ResponseEntity.status(HttpStatus.OK).body(CommonApiResponse.success(results));
    }
}
